...
```mermaid
flowchart TD
    KEY["Key astronomical moments"]
    KEY --> C["Chaitra dawn<br/>(before dawn, first day)"]
    KEY --> A["Sun enters Aries<br/>(astronomical transition)"]
    
    C --> OBS1["Observe tattva + svara"]
    A --> OBS2["Observe tattva + svara"]
    
    OBS1 --> FORE["Yearly forecast"]
    OBS2 --> FORE
    
    FORE --> RAIN["Rainfall"]
    FORE --> FOOD["Food supply"]
    FORE --> PRICE["Grain prices"]
    FORE --> SOC["Societal conditions"]
```
...
>Note: In the yoga path (see [Yoga path](mukti-yoga.md)), Sushumna serves as the central channel for spiritual ascent.
