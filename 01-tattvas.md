...
```mermaid
flowchart TD
    TAT["TATTVAS<br/>five elemental principles"]
    TAT --> AK["Akasha · Ether/Space"]
    TAT --> VA["Vayu · Air"]
    TAT --> AG["Agni · Fire"]
    TAT --> JA["Jala · Water"]
    TAT --> PR["Prithvi · Earth"]

    CRE["Creation / emanation order (§6-8):<br/>Akasha → Vayu → Agni → Jala → Prithvi"]
    FLOW["Svara-flow / dominance order (§72):<br/>Air → Fire → Earth → Water → Ether"]
    TAT -.how it emanates.- CRE
    TAT -.how it flows in breath.- FLOW
```
