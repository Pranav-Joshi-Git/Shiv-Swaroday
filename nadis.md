...
```mermaid
flowchart TD
    N["NADIS"] --> IDA["IDA · left · moon"]
    N --> SU["SUSHUMNA · centre"]
    N --> PIN["PINGALA · right · sun"]

    SU --> SUW["Worldly side:<br/>adverse for ordinary action · danger"]
    SU --> SUY["Yogic side:<br/>the path of liberation"]
```
...
Nadis are the infrastructure that connects [Prana](prana-vayus.md) to [Svara](svara.md) — the channels through which vital force travels before becoming observable as breath. Without understanding the three primary nadis, the entire predictive system makes no sense: when the text says "moon flows" or "sun flows," it means prana is moving through Ida or Pingala. When it says "check which side is active," it's asking you to observe which nadi is dominant.

Every prediction rule in [Bhukti Engine](bhukti-reading-the-world.md) assumes you know whether Ida or Pingala is active. The direction rules in [War](bhukti-combat.md) depend on nadi-direction correspondence. The entire [Yoga path](mukti-yoga.md) is the practice of shifting prana from Ida/Pingala (duality, embodiment, worldly life) into Sushumna (unity, transcendence, liberation).
