...
```mermaid
flowchart LR
    FLOW["KNOW THE FLOW (svara)"]
    FLOW --> READ["Read the world · interpret"]
    FLOW --> TRANS["Transform the self · control"]
    READ --> BH["BHUKTI · worldly mastery"]
    TRANS --> MU["MUKTI · liberation"]
    BH --> ATM["ATMAN / JIVA"]
    MU --> ATM
    ATM --> PARA["PARAMATMA"]
    PARA --> MOK["MOKSHA"]
...
The text acknowledges this: "If, suddenly, the lunar svara flows during the flow of solar svara or vice-versa then one must ask about the consequence from the Guru only. The Vedas and crores of other Shastra are of no avail in this regard" `§301`. See [Conception](bhukti-conception.md) for full analysis.
