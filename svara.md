...
```mermaid
flowchart LR
    SVARA["SVARA · breath-flow"]
    SVARA --> LU["Lunar (Ida)"]
    SVARA --> SO["Solar (Pingala)"]
    SVARA --> SH["Shoonya (Sushumna)"]

    SVARA --> POL["POORNA / SHOONYA"]
    POL --> PO["Poorna · full / active side"]
    POL --> SHO["Shoonya · empty / closed side"]
    PO -.->|"if svara unknown"| FL["Drop a flower →<br/>side it falls = Poorna"]
```
...
This variable stack feeds into both wings of the system: the **bhukti** (worldly prediction) methods detailed in [Prediction engine](bhukti-reading-the-world.md), and the **mukti** (liberation) practices detailed in [Yoga path](mukti-yoga.md).
...
Every prediction method in the bhukti chapters depends on observing the svara at the moment the question is asked. [War outcomes](bhukti-combat.md) depend on which svara is active when the armies meet. [Disease prognosis](bhukti-disease-prognosis.md) depends on observing irregular svara patterns. [Conception predictions](bhukti-conception.md) depend on which partner's svara is dominant during union. Without the ability to read svara, the entire predictive engine is unusable.
