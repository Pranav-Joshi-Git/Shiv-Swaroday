...
```mermaid
flowchart TD
    SHIFT["System shift:<br/>observation → control<br/>prediction → transcendence"]
    SHIFT --> INN["Inner rivers:<br/>Ida = Ganga<br/>Pingala = Yamuna<br/>Sushumna = Saraswati"]
    
    INN --> PRAN["PRANAYAMA<br/>(three breaths)"]
    PRAN --> PU["Puraka<br/>(inhale)"]
    PRAN --> KU["Kumbhaka<br/>(retain)"]
    PRAN --> RE["Rechaka<br/>(exhale)"]
    
    KU --> CROSS["Cross-nadi circulation:<br/>inhale Solar → retain → exhale Lunar"]
    
    CROSS --> MUD["Shanmukhi Mudra<br/>(sensory withdrawal)"]
    MUD --> TATT["Direct tattva perception"]
    
    CROSS --> VAY["Prana + Apana union<br/>rise via Sushumna"]
    VAY --> SAH["Sahasrara"]
    
    SAH --> SAM["SAMADHI<br/>(thoughtless 3 hours)"]
    SAM --> VIC["Victory over death"]
    SAM --> SID["Siddhis"]
    SAM --> ATM["Atman → Paramatma → Moksha"]
```
...
This is the method referenced in [Tattvas](tattvas.md) for directly perceiving the elemental forms (circle, crescent, triangle, dot) and qualities.
...
This is the yogic resolution to the death prognosis given in [Death](bhukti-lifespan-prognosis.md). Instead of predicting death through diagnostic signs, the practitioner transcends death through pranayama and mental steadiness.
