...
```mermaid
flowchart LR
    WAR["WAR OUTCOME"] --> ROLE{"Who is asking?"}
    ROLE -->|"Defender (invaded)"| DEF["Ida flows → Victory"]
    ROLE -->|"Invader (attacking)"| INV["Pingala flows → Victory"]
    ROLE -->|"Either role"| SUS["Sushumna flows → Danger"]

    WAR --> TAT["+ Tattva:<br/>Water=win · Fire=injury · Air/Ether=death"]
    WAR --> DIR["+ Face direction matching<br/>your active svara → Victory"]
    WAR --> POS["+ Opponent on Shoonya side → Controlled<br/>Opponent on Poorna side → Escapes control"]
    WAR --> QN["+ Question during Ida → Compromise<br/>Question during Pingala → Battle happens"]
```
...
**Letter parity (from [Reading the World](bhukti-reading-the-world.md)):**
...
The same techniques used here for tactical advantage — switching svaras, holding breath, aligning with direction — are the foundation for the yogic practices in [Yoga path](mukti-yoga.md), where the objective shifts from winning confrontations to transcending the confrontational mode of existence entirely.
